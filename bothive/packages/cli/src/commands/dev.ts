...
function getChatHTML(botName: string, botDescription: string) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
export async function devCommand(options: DevOptions) {
    ...
    const spinner = ora({ text: 'Loading bot...', color: 'gray' }).start();
    ...
    try {
        ...
        spinner.succeed(chalk.dim('Bot loaded'));
            ...
            if (req.method === 'POST' && req.url === '/run') {
                ...
                req.on('end', async () => {
                    try {
                        ...
                        console.log(chalk.white(`  < ${response.substring(0, 60)}${response.length > 60 ? '...' : ''}`));
        ...
        server.listen(port, () => {
            console.log(`
${chalk.dim('─────────────────────────────────────')}
${chalk.white.bold('  DEV SERVER')}
${chalk.dim('─────────────────────────────────────')}

  ${chalk.dim('Bot:')}      ${chalk.white(projectConfig.name)}
  ${chalk.dim('Chat:')}     ${chalk.cyan(`http://localhost:${port}`)}
  ${chalk.dim('API:')}      ${chalk.dim(`POST /run`)}
...
async function generateResponse(prompt: string, botCode: string, config: any): Promise<string> {
