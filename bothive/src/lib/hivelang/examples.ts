...
bot StudyBuddySwarm
  ...
  # memory currentSubject, studyPlan, progressTracker, studyStreak
  ...
  agent KnowledgeAgent
    on input
      if task == "create_schedule"
        say "🔍 Searching for Notion databases..."
        call notion.get_databases as dbs

        if dbs.length == 0
           say "⚠️ No Notion databases found. Please create one first."
           return null
        end

        set targetDb to dbs[0]
        say "Found " + dbs.length + " databases. Using: " + targetDb.title

        call notion.create_page with parent_id: targetDb.id, title: "Study Schedule: " + topic, content: "Schedule for " + topic + "\\n\\n[Generated by Study Buddy]" as page

        say "✅ Created schedule page: " + page.url
        return page
      end
    end
  end
