...
const EXAMPLES = [
  ...
  on input
    ...
    },
    {
        ...
        description: "Run independent tasks concurrently to speed up response times using the `parallel` block.",
        icon: Zap,
        code: `bot "FastResearcher"
  on input
    say "Researching topic: " + input.topic

    parallel
      // Task 1: Search the web
      call google.search(
        query: input.topic + " latest news"
      ) as search_results

      // Task 2: Search internal Notion DB
      call notion.search(
        query: input.topic
      ) as internal_docs
    end

    // Use both results after they finish
    call ai.generate(
      prompt: "Synthesize these findings: " + search_results + internal_docs
    ) as summary
    ...
    {
        title: "3. AI & Chain of Thought",
        description: "Generate text, analyze sentiment, or make decisions using the native AI capabilities.",
        icon: Cpu,
        code: `bot "WriterBot"
  on input
    // Direct generation
